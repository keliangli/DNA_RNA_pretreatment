...
def aaindex_feature_svm_test(RNA_last_row, RNA_last_column):
    ...
    predict_result = 0
    while i < 10:
        X_train, X_test, y_train, y_test = cross_validation.train_test_split(data_content, data_target, test_size=0.25,
                                                                             random_state=33)
...
def match_aaindex_content_value(file_content):
    ...
    if CA_value == None:
        ...
        match_aaindex_value_list = [CA_value.group(1), CA_value.group(2), CA_value.group(3), CA_value.group(4),
                                    CA_value.group(5),
                                    CA_value.group(6), CA_value.group(7), CA_value.group(8), CA_value.group(9),
                                    CA_value.group(10)]
        ...
        if file_aaindex_content != '\n':
            ...
            for file_content in file_handle:
                ...
                if extract_aaindex_attribute_flag == 1:
                    ...
                    aaindex_attribute_list = aaindex_attribute_list1 + aaindex_attribute_list2
                    aaindex_list_to_dict(aaindex_attribute_list, aaindex_attribute_dict)
                    DNA_last_row = PDB_aaindex_feature_Function_DNA(PDB_DNA_Shape_Path, aaindex_attribute_dict,
                                                                    aaindex_attribute_num)
                    RNA_last_row = PDB_aaindex_feature_Function_RNA(PDB_RNA_Shape_Path, aaindex_attribute_dict,
                                                                    DNA_last_row, aaindex_attribute_num)
