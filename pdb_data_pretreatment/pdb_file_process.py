...
def Match_PDB_Calcu_DX(PDB_Content_Atom):
    ATOM_m = re.match('ATOM\s+[0-9]+\s+\w+\s+(DA|DT|DG|DC|C|G|A|U)\s+\w+\s+\d+\s+(\D?\d+\.\d+)\s+(\D?\d+\.\d+)\s+(\D?\d+\.\d+).+',PDB_Content_Atom)
    if ATOM_m == None:
        return 0
    else:
...
def Print_PDB_Amino_acid_SI(pdb_file_name,CA_ATOM_SI_Value,CA_ATOM_Coordinate,Residues_Shape,CA_interface_result):
    ...
    Write_file_handle.write("  ca_dx_dis:")
    Write_file_handle.write(str(CA_interface_result))
...
def PDB_File_Process(pdb_file_list):
                   ...
                   continue
                    ...
                    if CA_interface_result :
                        ...
                        if CA_ATOM_SI_Value<-0.2:
                            ...
                            Residues_Shape = "peak"
                        Print_PDB_Amino_acid_SI(pdb_file_name,CA_ATOM_SI_Value,CA_ATOM_Coordinate,Residues_Shape,CA_interface_result)
