...
def match_dssp_content(dssp_file_content):
    ...
    Shape_Type_m = re.match('\s+\d+\s+(\d+)\s([A-Z])\s([A-Z]).{20}\s{1,3}(\d+).*',dssp_file_content)
    if Shape_Type_m== None:
        return 0
    else:
        Shape_Type = (Shape_Type_m.group(1),Shape_Type_m.group(2),Shape_Type_m.group(3),Shape_Type_m.group(4))
        return  Shape_Type
...
def calcu_relative_solvent_sccessiblity():
    ...
    for dssp_file in PDB_Path_listdir:
        ...
        for dssp_file_content in dssp_file_handle:
            ...
            if dssp_content:
                print(dssp_content)
                if realtive_ACC_calcu(dssp_content) > 0.36:
